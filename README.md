...
- 内置默认开启的浏览器 Agent：AI 根据页面可操作元素生成并执行点击、输入、选择、滚动等操作计划。
- 侧边栏采用简洁输入控制台，可在输入框下方切换三档 Agent 权限：默认权限、自动审查、完全访问权限。
- Agent 操作后会主动刷新页面上下文；若操作触发页面跳转，会在新页面自动恢复侧边栏。
...
在侧边栏直接输入目标，例如“帮我搜索 BrowserMate AI”或“打开设置里的高级选项”，BrowserMate AI 会判断这是页面操作任务，采集当前页面中可见的按钮、链接、输入框、下拉框等可操作元素摘要，并发送给你配置的 AI 服务生成操作计划。

也可以从扩展 Popup 直接进入“浏览器 Agent”，此时侧边栏会聚焦到页面任务输入。

Agent 权限有三档：

- 默认权限：所有操作计划都先展示，用户确认后执行。
- 自动审查：低风险操作自动执行，提交、保存、发布等敏感操作仍需确认。
- 完全访问权限：支持的页面动作自动执行，但支付、购买、下单、删除、转账、上传文件、输入密码等高风险动作仍会被拦截。

当前支持的动作包括点击、输入、选择、勾选、取消勾选、滚动和等待。

当操作导致页面内容变化、SPA 路由变化或普通页面跳转时，BrowserMate AI 会清理旧的正文缓存并分阶段重新读取页面。若完整跳转重新加载了内容脚本，扩展会在短时间内恢复当前标签页的侧边栏打开状态，避免 Agent 会话中断。
